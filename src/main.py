...
# Exact match
def exact_match(reports: list[Report], look_in: str = "impression", check_synonyms: bool = False) -> list[str]:
    ...
    for report in tqdm(reports):
        ...
        if not found_path:
            preds.append("UNKNOWN/NO PATHOLOGY")
            report.pred_patholoNgy = "UNKNOWN/NO PATHOLOGY"
...
def fuzzy_match(reports: list[Report], look_in: str = "impression", threshold: float = 80.0) -> list[str]:
    ...
    for report in tqdm(reports):
        ...
        if not found_path:
            preds.append("UNKNOWN/NO PATHOLOGY")
            report.pred_pathology = "UNKNOWN/NO PATHOLOGY"
...
print(f"Number of unlabeled reports with exact matching in the impression: {c_exat_impression['UNKNOWN/NO PATHOLOGY']}")

preds_exact_whole_report = exact_match(reports, "report")
c_exact_whole_report = Counter(preds_exact_whole_report)
print(f"Number of unlabeled reports with exact matching in the whole report: {c_exact_whole_report['UNKNOWN/NO PATHOLOGY']}")

# Fuzzy match
preds_fuzzy_impression = fuzzy_match(reports, "impression", threshold=90)
c_fuzzy_impression = Counter(preds_fuzzy_impression)
print(f"Number of unlabeled reports with fuzzy matching in the impression: {c_fuzzy_impression['UNKNOWN/NO PATHOLOGY']}")

preds_fuzzy_whole_report = fuzzy_match(reports, "report", threshold=90)
c_fuzzy_whole_report = Counter(preds_fuzzy_whole_report)
print(f"Number of unlabeled reports with fuzzy matching in the whole report: {c_fuzzy_whole_report['UNKNOWN/NO PATHOLOGY']}")


## Check with synonyms
preds_exact_impression_synonyms = exact_match(reports, "impression", check_synonyms=True)
c_exat_impression_synonyms = Counter(preds_exact_impression_synonyms)
print(f"Number of unlabeled reports with exact matching in the impression with synonyms: {c_exat_impression_synonyms['UNKNOWN/NO PATHOLOGY']}")

preds_exact_whole_report_synonyms = exact_match(reports, "report", check_synonyms=True)
c_exact_whole_report_synonyms = Counter(preds_exact_whole_report_synonyms)
print(f"Number of unlabeled reports with exact matching in the whole report with synonyms: {c_exact_whole_report_synonyms['UNKNOWN/NO PATHOLOGY']}")
